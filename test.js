...
console.log('categories',categories.map(c=>({value:c, label: c.charAt(0).toUpperCase() + c.slice(1).toLowerCase().split('-').join(" "), checked: false})))

const brands = [...new Set([...products.map(product => product.brand)])]
console.log('categories',brands.map(c=>({value:c, label: c.split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(" "), checked: false})))